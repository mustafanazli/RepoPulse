...
**Bilinen, kasıtlı olarak düzeltilmemiş kısıt (gelecek RP adayı):** Sistem tarayıcısı GitHub'a hiç ulaşamadığında veya kullanıcı OAuth'u tamamlamadan geri döndüğünde, `LoginPage`'in giriş butonu uygulama yeniden başlatılana kadar devre dışı kalıyor. Bu bir çökme, veri sızıntısı veya yarım oturum DEĞİL — yalnızca kullanıcı deneyimi kısıtı. Doğru düzeltme `MainActivity.OnResume` + yeni bir `OAuthCallbackBroker` sinyali gerektirir; gerçek bir başarılı exchange ile yarışmaması için dikkatli tasarım gerekiyor. Bu RP'nin kapsamı dışında bırakıldı.
...
- [x] Ağ bağlantısı kesikken (uçak modu) istek denemesi çöküşe yol açmıyor, hata durumu ekranda gösteriliyor — **RP-010/RP-012/RP-013/RP-014 ile karşılandı**
  - **Kanıt durumu (RP-014 ile güncellendi, 2026-08-28):** Altı ana ekranın tamamı için ekran bazında kanıt:
    ...
    - Tek repository araması (manuel lookup) — **doğrudan canlı kanıt: RP-014** (ağ kapalıyken "GitHub'a ulaşılamadı." güvenli mesajı, liste/favoriler bozulmadı, çökme yok; ağ açılınca retry başarılı)
    - Giriş/OAuth akışı — **RP-014 ile karışık kanıt**: RepoPulse'un kendi HTTP istekleri (`ExchangeAsync`/`GetCurrentUserAsync`) artık hem tipli hem de zaman-aşımı/iptal kaynaklı hatalarda çökmeden güvenli mesaj gösteriyor (canlı doğrulandı — gerçek bir AuthApi `NetworkError`i "Sunucuya ulaşılamadı." ile güvenli şekilde ele alındı, retry başarılı oldu), ve `BootstrapPage`'in soğuk başlatma restore akışı hiç ağ çağrısı yapmıyor (RP-008). **Bilinen, ayrı bir kısıt** (kriterin "istek denemesi" kapsamı dışında, aşağıdaki RP-014 girişine bkz.): sistem tarayıcısı GitHub'a hiç ulaşamadığında/kullanıcı geri döndüğünde giriş butonu uygulama yeniden başlatılana kadar devre dışı kalabiliyor — çökme veya veri sızıntısı değil, kasıtlı olarak düzeltilmemiş bir UX kısıtı, gelecek RP adayı
