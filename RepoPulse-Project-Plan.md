...
**RP-014 düzeltmesi — Cancelled/Invalid outcome doğrulama açığı — TAMAMLANDI.** Yukarıdaki denetim Success çıktısını doğru sıraya soktu, ama Cancelled/Invalid dallarını hedefli ikinci bir turda incelendiğinde gerçek bir açık daha bulundu: bu dallarda `coordinator.TryConsumeCallback(currentAttemptId)` HİÇBİR state doğrulaması yapılmadan çağrılıyordu — çünkü `OAuthCallbackParser` o zamana kadar Cancelled/Invalid çıktıları için state'i tamamen atıyordu. Sonuç: terk edilmiş A'nın gecikmeli bir `access_denied` veya başka bir OAuth error'ı, B aktifken gelirse, `currentAttemptId=B` olduğundan `TryConsumeCallback(B)` başarıyla B'yi terminal yapıyor ve ardından `sessionStore.Reset()` B'nin hâlâ geçerli PKCE session'ını telafisiz siliyordu — B'nin gerçek callback'i daha sonra gelse bile artık reddediliyordu. Kök neden: GitHub'ın RFC 6749 4.1.2.1 gereği cancellation/error yönlendirmelerinde de bizim gönderdiğimiz state'i geri yansıttığı, ama `OAuthCallbackParser`'ın bunu hiç okumadan attığı. Düzeltme: `OAuthCallbackResult.Cancelled`/`Invalid` artık opsiyonel bir `state` parametresi taşıyor, `OAuthCallbackParser` bunu error/eksik-code-veya-state dallarında koruyor; `OAuthCallbackAttemptGate.Evaluate`, Success ile birebir aynı desende — Cancelled/Invalid için de ÖNCE `AuthorizationSessionStore.TryConsume(state)` çağırıyor (state yoksa veya eşleşmiyorsa anında `Ignored`, hiçbir dokunma olmadan), yalnızca state genuinely eşleştiğinde coordinator'ı terminal yapıp session'ı sonlandırıyor. `AuthorizationSessionStore`'a yeni bir API eklenmedi — `TryConsume` zaten token/verifier döndürmeden yalnızca doğrulayıp tüketen dar sözleşmeyi karşılıyordu. Ham `error`/`error_description` hâlâ hiçbir yerde okunmuyor/loglanmıyor (yalnızca sınıflandırma + `State` kullanılıyor). 13 yeni gerçekçi test eklendi (`OAuthCallbackOutcomeValidationTests`, aynı dosyada) — eski A'nın gerçek `access_denied`/başka OAuth error'ı B aktifken B'yi asla iptal/sıfırlamıyor, eksik/yanlış/bozuk state'li callback'ler B'yi asla tüketmiyor, B'nin kendi geçerli cancellation/error'ı B'yi tam bir kez sonlandırıyor, eski geçersiz sinyalden sonra B'nin gerçek callback'i yine de tam başarıyla tamamlanıyor, gerçek eşzamanlı cancellation-vs-resume ve success-vs-cancellation yarışlarında tam olarak tek sonuç üretiliyor (`AuthorizationSessionStore`'un kendi tek-kullanımlık tüketimi ile garantili), geçersiz callback'in exchange başlatamadığı ve başka bir denemenin session'ını asla silemediği, ham error/error_description/state değerlerinin decision/session yüzeyine hiç sızmadığı (benzersiz işaretleyici string'lerle kanıtlandı) doğrulanıyor — hiçbiri callback'e yapay bir attempt id geçirmiyor. Önceki turun 1 testi (`InvalidCallbackNeverCreatesOrPersistsSession`) artık düzeltilmiş (daha güvenli) davranışı yansıtacak şekilde güncellendi: state'siz bir Invalid callback artık `AttemptEndedSafely` değil `Ignored` üretiyor. 471/471 `RepoPulse.UnitTests` (456'ya +15: 13 entegrasyon testi + 2 parser testi) ve 51/51 `RepoPulse.AuthApi.Tests` geçiyor; yeni NuGet paketi eklenmedi; timeout/`OperationCanceledException` güvenliği, terk edilmiş tarayıcı kurtarma ve event/lifetime davranışları değişmeden korundu.

**RP-015 — Aktivite alt puanı: son commit güncelliği — TAMAMLANDI.** Faz 2 / issue `#13`'ün (alt puan hesaplama kuralları) yalnızca en küçük dikey dilimini kapatır: Aktivite alt puanının **yalnızca** "son commit'in güncelliği" bileşeni. Issue `#13`'ün tamamı, Aktivite alt puanının bütünü (commit sıklığı ve trend hâlâ tamamen açık), diğer dört alt puan (Bakım/CI-CD/Dokümantasyon/Topluluk), ağırlıklı Genel Sağlık Puanı ve her türlü UI/API/SQLite bağlantısı bu turun kapsamı dışında bırakıldı — hiçbiri tamamlanmış sayılmıyor. §6'daki plan metni Aktivite için yalnızca nitel maddeler listeliyor (kesin eşik/puan tanımlamıyor), bu yüzden bu turda aşağıdaki RP-015 v0.1.0 eşik/puan politikası tanımlanıp kaydedildi:

| Son commit yaşı | Puan | Bant |
|---|---|---|
| 0–7 gün (`age <= 7g`) | 100 | `Fresh` |
| 7 günden büyük, 30 güne kadar (`7g < age <= 30g`) | 75 | `Recent` |
| 30 günden büyük, 90 güne kadar (`30g < age <= 90g`) | 40 | `Aging` |
| 90 günden büyük (`age > 90g`) | 10 | `Stale` |
| GitHub'ın doğruladığı "commit yok" durumu | 0 | `NoCommits` |

Mimari: yeni `RepoPulse.Core.Scoring` alanı — MAUI/API/SQLite'tan tamamen bağımsız, saf `ActivityRecencyScorer.Score(DateTimeOffset? lastCommitUtc, DateTimeOffset nowUtc)`; sistem saatini hiçbir yerde kendi okumuyor (`DateTime.Now`/`DateTimeOffset.Now`/`.UtcNow` çağrısı yok, bunu doğrulayan kaynak-taramalı bir test de eklendi) — "şimdi" her zaman çağıran tarafından veriliyor, böylece tamamen deterministik ve gerçek zaman beklemeden test edilebilir. Sonuç modeli `ActivityRecencyScore(Value, Band, AlgorithmVersion)` — immutable `record`, token/session/API-client şeklinde hiçbir alan taşımıyor (reflection testiyle kanıtlandı). **Null semantiği açıkça sınırlandı:** `lastCommitUtc == null` YALNIZCA "GitHub repository'de commit olmadığını doğruladı" anlamına gelir — API hatası/rate-limit/ağ hatası/malformed response gibi "veri alınamadı" durumları asla null'a indirgenip buraya geçirilmemeli; üst katman bu tür hataları kendi ayrı failure state'inde tutup scorer'ı hiç çağırmamalı. **Gelecek tarihli commit politikası:** Git commit metadata'sındaki saat, yerel saat kontrolünde değildir — `lastCommitUtc`, `nowUtc`'den ileride çıkarsa (bozuk saat) exception atılmıyor, yaş sıfıra clamp edilip `Fresh`/100 olarak değerlendiriliyor; bu, saat sapmasının asla "bozuk/durgun repository" kanıtı sayılmaması için kasıtlı bir tercih. Algoritma sürümü: `ActivityRecencyScorer.AlgorithmVersion = "0.1.0"` (component id: `activity-recency`) — derlemeden/build zamanından bağımsız sabit bir literal; her `ActivityRecencyScore` sonucu bu sürümü taşıyor. 29 yeni unit test (`ActivityRecencyScorerTests.cs`) — tüm sınır değerleri (tam 7/30/90 gün ve her birinin +1 tick'i) yaklaşıklık/yuvarlama olmadan `TimeSpan` hassasiyetiyle sabitleniyor, null→`NoCommits`/0, gelecekteki commit→clamp+`Fresh`/100, farklı UTC offset'leriyle aynı anın aynı puanı ürettiği, aynı girdilerin deterministik sonuç ürettiği, `Value`'nun her senaryoda 0–100 aralığında kaldığı, `AlgorithmVersion`'ın tam olarak `"0.1.0"` olduğu, bant-puan eşleşmesi, sistem saatinin hiç okunmadığının kaynak taramasıyla kanıtı, null-sözleşmesinin kaynakta dokümante edildiğinin kanıtı, ve sonuç/scorer tiplerinde token/session/client şeklinde alan bulunmadığı doğrulanıyor. 500/500 `RepoPulse.UnitTests` (471'e +29) ve 51/51 `RepoPulse.AuthApi.Tests` geçiyor; yeni NuGet paketi eklenmedi; GitHub API/UI/SQLite/Azure/AuthApi/GHCR katmanlarına dokunulmadı — Core-only olduğu için emülatör/canlı staging doğrulaması bu turda yapılmadı (gerekmiyor). Faz 2'nin genel çıkış kriterleri henüz `[ ]` — bu yalnızca tek bir alt bileşenin temelini kuruyor.
...
Issue'lar:
- `#13` Domain katmanında alt puan hesaplama kuralları (Aktivite, Bakım, CI/CD, Dokümantasyon, Topluluk) — API istemcisinden bağımsız saf fonksiyonlar olarak — **kısmen: Aktivite'nin yalnızca "son commit güncelliği" bileşeni RP-015 ile TAMAMLANDI** (commit sıklığı/trend + diğer dört alt puan hâlâ açık)
